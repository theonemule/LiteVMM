...
${commandLine(ci)} < user-data`;
  ...
  async function loadVolumes(){
    ...
    $('#view').innerHTML=card('Docker volumes',`<div class="small text-secondary mb-3">Docker's local volume driver can mount ordinary local storage plus Linux NFS, CIFS/SMB, bind mounts, and tmpfs. Custom drivers and driver options remain available.</div>${table(['Name','Driver','Scope',''],rows,'No Docker volumes found.')}`,`<button class="btn btn-sm btn-primary" id="createVolBtn">Create volume</button>`);
    $('#createVolBtn').onclick=()=>{
      modal({eyebrow:'Docker storage',title:'Create volume',submitText:'Create',body:`<form id="volForm"><div class="row g-3"><div class="col-md-6"><label class="form-label">Name</label><input name="name" class="form-control" required></div><div class="col-md-6"><label class="form-label">Storage type</label><select name="kind" id="volumeKind" class="form-select"><option value="local">Docker local volume</option><option value="bind">Bind host directory</option><option value="nfs">NFS share</option><option value="cifs">SMB / CIFS share</option><option value="tmpfs">tmpfs memory volume</option><option value="custom">Custom driver/options</option></select></div></div><div id="volumeFields" class="mt-3"></div></form>`,onSubmit:async(el,m)=>{const form=$('#volForm',el),fd=new FormData(form),kind=fd.get('kind'),o={name:fd.get('name')};const opts=[];if(kind==='bind'){o.driver='local';opts.push('type=none','o=bind',`device=${fd.get('device')}`);}else if(kind==='nfs'){o.driver='local';opts.push('type=nfs',`o=addr=${fd.get('server')},rw,nfsvers=${fd.get('version')||'4'}`,`device=:${fd.get('path')}`);}else if(kind==='cifs'){o.driver='local';const auth=[`addr=${fd.get('server')}`,`vers=${fd.get('version')||'3.0'}`,'rw'];if(fd.get('username'))auth.push(`username=${fd.get('username')}`);if(fd.get('password'))auth.push(`password=${fd.get('password')}`);o.driver='local';opts.push('type=cifs',`o=${auth.join(',')}`,`device=//${fd.get('server')}/${String(fd.get('share')||'').replace(/^\/+/, '')}`);}else if(kind==='tmpfs'){o.driver='local';opts.push('type=tmpfs','device=tmpfs',`o=size=${fd.get('size')||'256m'}`);}else if(kind==='custom'){o.driver=fd.get('driver')||'local';String(fd.get('options')||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean).forEach(x=>opts.push(x));}else{o.driver='local';}opts.forEach((v,i)=>o[`opt_${i}`]=v);await request('/docker/volumes',{method:'POST',form:o});m.hide();toast(`${o.name} created`);await renderRoute();}});
      const form=$('#volForm',$('#formModal')),fields=$('#volumeFields',form);const sync=()=>{const k=form.kind.value;fields.innerHTML=k==='bind'?`<label class="form-label">Host directory</label><input name="device" class="form-control mono" placeholder="/srv/data" required>`:k==='nfs'?`<div class="row g-3"><div class="col-md-5"><label class="form-label">NFS server</label><input name="server" class="form-control mono" placeholder="10.0.0.20" required></div><div class="col-md-5"><label class="form-label">Export path</label><input name="path" class="form-control mono" placeholder="/exports/app" required></div><div class="col-md-2"><label class="form-label">NFS version</label><input name="version" class="form-control" value="4"></div></div>`:k==='cifs'?`<div class="alert alert-warning small">Docker stores local-driver mount options in volume metadata. If you enter an SMB password here it can be visible to Docker administrators through volume inspection.</div><div class="row g-3"><div class="col-md-4"><label class="form-label">SMB server</label><input name="server" class="form-control mono" required></div><div class="col-md-4"><label class="form-label">Share</label><input name="share" class="form-control" required></div><div class="col-md-4"><label class="form-label">SMB version</label><input name="version" class="form-control" value="3.0"></div><div class="col-md-6"><label class="form-label">Username</label><input name="username" class="form-control"></div><div class="col-md-6"><label class="form-label">Password</label><input name="password" type="password" class="form-control"></div></div>`:k==='tmpfs'?`<label class="form-label">Maximum size</label><input name="size" class="form-control" value="256m"><div class="form-text">Linux tmpfs size such as 256m or 2g.</div>`:k==='custom'?`<label class="form-label">Driver</label><input name="driver" class="form-control mb-3" value="local"><label class="form-label">Driver options</label><textarea name="options" rows="5" class="form-control mono" placeholder="type=nfs\no=addr=10.0.0.20,rw,nfsvers=4\ndevice=:/exports/app"></textarea><div class="form-text">One Docker <span class="mono">--opt</span> value per line.</div>`:'<div class="small text-secondary">Docker manages storage under its normal local volume path.</div>';};form.kind.addEventListener('change',sync);sync();
    };
  ...
  async function loadBackups() {
    if (!hasCap('backup-create')) {
      const [backups, peers, replicas] = await Promise.all([request('/backups'), request('/cluster/peers'), hasCap('replication-receiver')?request('/replications/replicas').catch(()=>[]):Promise.resolve([])]);
    ...
    const [backups, schedules, vms, peers, replicas] = await Promise.all([request('/backups'), request('/backups/schedules'), request('/vms'), request('/cluster/peers'), hasCap('replication-receiver')?request('/replications/replicas').catch(()=>[]):Promise.resolve([])]);
