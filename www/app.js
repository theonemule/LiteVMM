...
${commandLine(ci)} < user-data`;
  ...
  async function loadAdmin() {
    ...
    const mismatch=endpointLooksHttps&&!tls?`<div class="alert alert-warning small mb-3">The browser is connected with HTTPS but LiteVMM's persisted TLS state says HTTPS is disabled. This is legacy/stale state; current storage images reconcile a persisted managed certificate at startup.</div>`:'';
