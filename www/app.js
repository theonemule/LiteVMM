...
${commandLine(ci)} < user-data`;
  ...
  async function openVMDiskStorage(){
    ...
    modal({eyebrow:'VM storage',title:'Virtual disks',size:'xl',body:`<div class="alert alert-light border small">This inventory includes VM disks across the current host and paired hosts. New and imported disks can live locally or directly on peer-backed storage through the existing NFS/WSS backplane.</div>
  ...
  async function openDockerImageLibrary(){
    ...
    const body=`${errors}
      <div class="alert alert-light border small">This federated library combines Docker images on this host and every directly paired host without replicating them. A peer-only image remains on its owning host. Containers can use that image as a read-only root filesystem over the NFS/WSS backplane while keeping only their writable layer on <strong>${esc(activeHost().name)}</strong>. <strong>Pull image</strong> explicitly creates a normal local Docker copy.</div>
  ...
  async function collectReplicationInventory() {
    ...
    const rows=sources.rows.map(item=>{
      ...
      const recoverable=disks.length>0?disks.every(d=>d.recoverable===true):(replicas.length>0&&replicas.every(d=>d.recoverable===true));
      ...
      return {
        ...
        state:stateName,recoverable,disk_count:Math.max(disks.length,replicas.length),bytes:configuredBytes||hostedBytes,
    ...
    for(const [k,replicas] of hostedGroups){
      ...
      rows.push({
        ...
        recoverable:replicas.every(d=>d.recoverable===true),disk_count:replicas.length,bytes:replicas.reduce((n,d)=>n+Number(d.bytes||0),0),
  ...
  function hostedPeerVolumeCard(items=[]) {
    ...
    return `<div class="mt-3">${card('Hosted peer volumes',`<div class="small text-secondary mb-3">These directories live inside the shared NFS storage backplane. Paired Docker hosts reach them through their peer-wide WSS tunnel; there is no per-volume transport or share process.</div>${table(['Volume / peer','State','Data size','Backing path',''],rows,'No peer volumes are hosted on this node.')}`)}</div>`;
  }
  ...
  async function loadBackups() {
    ...
    const renderBackupRows=()=>{
      ...
      $$('[data-backup-restore]').forEach(btn=>btn.onclick=()=>{
        ...
        confirmAction('Restore backup',`Restore ${archive} to its source VM ${vm} on ${targetLabel}? If ${vm} already exists it must be stopped and its current disks/configuration will be replaced after a rollback copy is created.`,async()=>{
    ...
    const renderReplicationRows=()=>{
      ...
      const rows=filtered.map(r=>{
        ...
        if(r.source){
          const stopped=r.source.running===false;
          const restoreButton=stopped&&r.recoverable?`<button class="btn btn-sm btn-outline-primary" data-replication-restore="${esc(r.vm)}" data-source-node="${esc(r.source_node_id||'')}" data-replica-node="${esc(r.replica_node_id||'')}">Restore to source</button>`:`<button class="btn btn-sm btn-outline-primary" disabled title="${esc(!stopped?'Stop the VM before restoring a continuous replica':'Replica has not completed initial synchronization')}">Restore to source</button>`;
          actions=`${restoreButton}<button class="btn btn-sm btn-outline-danger" data-replication-stop="${esc(r.vm)}" data-source-peer="${esc(r.source_peer_id||'')}">Stop</button>`;
        }else if(r.replicas?.length&&r.replicas.every(x=>x.active!==true)){
          const restoreButton=r.recoverable?`<button class="btn btn-sm btn-outline-primary" data-replication-restore="${esc(r.vm)}" data-source-node="${esc(r.source_node_id||'')}" data-replica-node="${esc(r.replica_node_id||'')}">Restore to source</button>`:`<button class="btn btn-sm btn-outline-primary" disabled title="Replica has not completed initial synchronization">Restore to source</button>`;
          actions=`${restoreButton}<button class="btn btn-sm btn-outline-danger" data-replication-purge="${esc(r.replicas.map(x=>x.id).join(','))}" data-replica-peer="${esc(r.replica_peer_id||'')}" data-replica-vm="${esc(r.vm)}">Purge retained</button>`;
        }
      ...
      $$('[data-replication-restore]').forEach(button=>button.onclick=()=>{
        const vm=button.dataset.replicationRestore,sourceNode=button.dataset.sourceNode||'',replicaNode=button.dataset.replicaNode||'';
        const localNode=String(state.hostCatalog.local?.node_id||'').toLowerCase();
        const targetPeer=sourceNode&&String(sourceNode).toLowerCase()!==localNode?sourceNode:'';
        if(!replicaNode){toast('Replica storage host is unavailable.','Restore unavailable');return;}
        confirmAction('Restore continuous replica',`Restore the latest retained replica back to ${vm} on ${nodeLabel(sourceNode)}? The VM must be stopped. Its current disks will be replaced, and the retained replica will remain available after the restore.`,async()=>{
          await request(`/replications/${encodeURIComponent(vm)}/restore`,{method:'POST',form:{peer_id:replicaNode},...(targetPeer?{peerId:targetPeer}:{local:true})});
          toast(`${vm}: replica restored to source`); await loadBackups();
        });
      });
  ...
  function bindBackupTarget(root) {
    const target=$('select[name="target"]',root); const sync=()=>{const peer=target.value==='peer';$('[data-backup-peer]',root)?.classList.toggle('d-none',!peer);$('[data-backup-local]',root)?.classList.toggle('d-none',peer);$('[data-backup-target-note]',root).textContent=peer?'The archive is written directly to the paired host through the shared NFS/WSS storage backplane and retained there only.':'Local folders can be mounted network shares.';};target.onchange=sync;sync();
  }
  ...
  async function beginHttpsTransition(status) {
    ...
    const showManual=()=>{
      ...
      manual.href=target.url;
    };
    const probe=()=>{
      if(finished)return;
      attempts++;
      if(attempts===10)showManual();
      const image=new Image();
      const timer=setTimeout(()=>{image.onload=image.onerror=null;setTimeout(probe,500);},1800);
      image.onload=()=>{
        clearTimeout(timer);finished=true;
        if(message)message.textContent='HTTPS is available. Redirecting…';
        window.location.replace(target.url);
      };
      image.onerror=()=>{clearTimeout(timer);setTimeout(probe,650);};
      image.src=`${target.health}?transition=${Date.now()}`;
    };
    probe();
  }

  function httpTransitionTarget(status) {
    let host=String(location.hostname||'').trim();
    if(host.includes(':')&&!host.startsWith('['))host=`[${host}]`;
    const port=Number(status.port||5186);
    const authority=`${host}${port===80?'':`:${port}`}`;
    return `http://${authority}${location.pathname}${location.search}${location.hash}`;
  }

  async function returnToHttp(status, removeCertificate=false) {
    const target=httpTransitionTarget(status);
    try {
      if(removeCertificate) await request('/admin/certificates/remove',{method:'POST',form:{}});
      else await request('/admin/certificates',{method:'DELETE'});
    } catch(error) {
      // The web listener may close the HTTPS socket while the request is being
      // completed. Do not replay the destructive action; navigate to HTTP.
    }
    toast(removeCertificate?'Certificate removed. Switching to HTTP…':'HTTPS disabled. Switching to HTTP…');
  ...
  async function loadAdmin() {
    ...
    const certificateKnown=Boolean(status.certificate||tls);
    const endpointLooksHttps=browserTransport==='HTTPS';
    const actions=`<div class="action-row justify-content-start">${certbotButton}<button id="generateCsrBtn" class="btn btn-outline-primary">Generate CSR</button><button id="importPairBtn" class="btn btn-outline-primary">Import certificate + key</button>${status.csr_available?'<button id="importSignedBtn" class="btn btn-outline-primary">Import signed CSR certificate</button>':''}${tls&&mode==='certbot'?'<button id="renewCertBtn" class="btn btn-outline-primary">Renew Let’s Encrypt</button>':''}${(tls||endpointLooksHttps)?'<button id="disableTlsBtn" class="btn btn-outline-danger">Disable HTTPS, keep certificate</button>':''}${(certificateKnown||endpointLooksHttps)?'<button id="removeTlsCertBtn" class="btn btn-danger">Remove certificate + use HTTP</button>':''}</div>`;
    ...
    const mismatch=status.tls_state_mismatch===true?`<div class="alert alert-warning small mb-3">LiteVMM is actively serving HTTPS, but the persisted TLS configuration does not match the live listener. The certificate shown here is detected from the runtime endpoint. Recreate or restart the storage container with the current image to reconcile the persisted state.</div>`:'';
    const retained=!tls&&status.certificate?'<div class="alert alert-info small mb-3">HTTPS is disabled, but the certificate is retained and can be replaced or removed.</div>':'';
    const enabledLabel=tls?'Yes':status.certificate?'No (certificate retained)':'No';
    $('#view').innerHTML=`<div class="row g-3 mb-3"><div class="col-xl-6">${card('Installation',systemDl([['Profile',svc.profile||status.profile],['API version',svc.version],['Management port',svc.port||status.port],['Transport',reloadRequired?`${browserTransport} (certificate reload pending)`:browserTransport],['HTTPS policy',status.tls_transport||'replace'],['HTTPS port',(status.tls_transport||'replace')==='replace'?(status.port||''):(status.https_port||'')],['Capabilities',(svc.capabilities||[]).join(', ')]]))}</div><div class="col-xl-6">${card('TLS certificate',systemDl([['Enabled',enabledLabel],['Management',mode],['Domain',status.domain||''],['Valid from',status.not_before||''],['Expires',status.expires||''],['Subject',status.subject||''],['Issuer',status.issuer||''],['SANs',status.sans||''],['SHA-256 fingerprint',status.fingerprint_sha256||'',true],['Certificate',status.certificate||'',true]]))}</div></div><div class="row g-3"><div class="col-xl-7">${card('Certificate management',`${mismatch}${retained}${reloadNotice}<div class="small text-secondary mb-3">Use Let’s Encrypt, a LiteVMM local CA, a CSR for an external CA, or import an existing PEM certificate/private-key pair. Certificate changes are staged first; HTTPS is activated only after you explicitly reload the endpoint.</div>${actions}`)}</div><div class="col-xl-5">${card('Pending CSR',pending)}</div></div>`;
    ...
    $('#selfSignedCertBtn')?.addEventListener('click',()=>modal({eyebrow:'HTTPS',title:'Issue self-signed certificate',submitText:'Issue certificate',body:`<form id="selfSignedCertForm"><label class="form-label">DNS name or IPv4 address</label><input name="domain" class="form-control mono mb-3" value="${esc(status.domain||'')}" placeholder="litevmm.local" required><label class="form-label">Additional DNS names or IPv4 addresses</label><input name="sans" class="form-control mono mb-3" placeholder="10.0.1.5,node.local"><label class="form-label">Lifetime in days</label><input name="days" type="number" min="1" max="36500" class="form-control" value="825"><div class="form-text mt-3">LiteVMM creates or reuses a local root CA and issues the endpoint certificate from it. Only the public CA certificate is shared during peer pairing. The CA private key remains on this host.</div></form>`,onSubmit:async(el,m)=>{const fd=new FormData($('#selfSignedCertForm',el));await request('/admin/certificates/self-signed',{method:'POST',form:Object.fromEntries(fd.entries())});m.hide();toast('Self-signed certificate is ready. Reload to activate HTTPS.');await loadAdmin();}}));
