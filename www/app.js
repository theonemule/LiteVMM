...
${commandLine(ci)} < user-data`;
  ...
  async function openVMReplication(name) {
    ...
    const disks=(status.disks||[]).map(d=>{const pct=d.length?Math.min(100,Math.round((d.offset||0)/d.length*100)):0;const state=d.ready?'Synchronized / continuous':(d.tunnel_running?'Initial sync':'Disconnected');return `<tr><td>disk${d.index}</td><td>${stateBadge(state)}</td><td>${d.ready?'100%':pct+'%'}</td><td>${bytes(d.offset||0)} / ${bytes(d.length||0)}</td><td>${d.tunnel_running?'WSS connected':'Tunnel down'}</td></tr>`;});
    const configuredBody=`<div class="alert alert-info small">The destination holds standby disk replicas. This is continuous block replication, not automatic VM failover or memory-state replication.</div><dl class="row small"><dt class="col-4 text-secondary">Peer</dt><dd class="col-8 mono">${esc(status.peer_id||'')}</dd></dl>${table(['Disk','State','Progress','Bytes','Transport'],disks,'No disk mirror jobs were reported.')}<div class="d-flex gap-2 mt-3"><button type="button" id="refreshReplicationBtn" class="btn btn-outline-secondary">Refresh</button><button type="button" id="stopReplicationBtn" class="btn btn-outline-danger">Stop replication</button></div>`;
    const startBody=`<div class="alert alert-light border small">Live replication performs an initial full disk synchronization and then mirrors subsequent writes over an authenticated WebSocket on the peer's existing LiteVMM HTTP(S) endpoint. No overlay network or public NBD port is used.</div><form id="replicationForm"><label class="form-label">Destination peer</label><select name="peer_id" class="form-select mb-3" required><option value="">Select paired host</option>${peerOptions}</select><label class="form-label">Optional bandwidth limit</label><input name="speed_mib" type="number" min="0" class="form-control" value="0"><div class="form-text">MiB/s. Use 0 for unlimited.</div></form>${vm.state!=='running'?'<div class="alert alert-warning small mt-3 mb-0">Start the VM before enabling live replication.</div>':''}`;
  ...
  async function loadVolumes(){
    const [vols,remoteMounts,peers]=await Promise.all([
      ...
      hasCap('remote-volume-client')?request('/docker/remote-volumes').catch(()=>[]):Promise.resolve([]),
      hasCap('remote-volume-client')?request('/cluster/peers').catch(()=>[]):Promise.resolve([])
    ]);
    ...
    $('#createVolBtn').onclick=async()=>{
      ...
      for(const peer of (peers||[]).filter(p=>p.url&&String(p.url).startsWith('https://'))){
        try { const svc=await request('/',{peerId:peer.node_id}); if((svc.capabilities||[]).includes('remote-volume-receiver')) eligible.push(peer); }
      ...
      modal({eyebrow:'Docker storage',title:'Create volume',submitText:'Create',body:`<form id="volForm"><div class="row g-3"><div class="col-md-6"><label class="form-label">Docker volume name</label><input name="name" class="form-control" required></div><div class="col-md-6"><label class="form-label">Storage type</label><select name="kind" id="volumeKind" class="form-select"><option value="local">Docker local volume</option>${hasCap('remote-volume-client')?'<option value="peer">LiteVMM peer volume over WSS</option>':''}<option value="bind">Bind host directory</option><option value="nfs">NFS share</option><option value="cifs">SMB / CIFS share</option><option value="tmpfs">tmpfs memory volume</option><option value="custom">Custom driver/options</option></select></div></div><div id="volumeFields" class="mt-3"></div></form>`,onSubmit:async(el,m)=>{
        ...
        if(kind==='peer'){
          ...
          if(!peer_id||!remote_name)throw new Error('Select a paired host and remote volume name.');
          await request('/docker/remote-volumes',{method:'POST',form:{peer_id,remote_name,name:o.name}});m.hide();toast(`${o.name} mounted from paired host`);await renderRoute();return;
        }
        const opts=[];
  ...
  function replicaInventoryTable(replicas=[]) {
    const rows=(replicas||[]).map(r=>{
      const healthy=r.active===true && r.nbd_running===true && r.websocket_running===true;
  ...
  function hostedRemoteVolumeCard(items=[]) {
    if(!hasCap('remote-volume-receiver'))return '';
    const rows=(items||[]).map(v=>`<tr><td><div class="mono">${esc(v.name||'')}</div><div class="small text-secondary mono">${esc(v.owner||'')}</div></td><td>${stateBadge(v.active?'available':'stopped')}</td><td>${bytes(v.bytes||0)}</td><td class="mono small text-break">${esc(v.data||'')}</td><td><div class="action-row">${v.active?`<button class="btn btn-sm btn-outline-secondary" data-hosted-volume-stop="${esc(v.id)}">Stop</button>`:`<button class="btn btn-sm btn-outline-danger" data-hosted-volume-purge="${esc(v.id)}">Remove share</button><button class="btn btn-sm btn-danger" data-hosted-volume-delete="${esc(v.id)}">Delete data</button>`}</div></td></tr>`);
    return `<div class="mt-3">${card('Hosted peer volumes',`<div class="small text-secondary mb-3">These directories are backing LiteVMM FUSE filesystems mounted by paired Docker hosts over this node's existing authenticated WSS endpoint. Stopping access preserves the data. Deleting data is an explicit separate action.</div>${table(['Volume / peer','State','Data size','Backing path',''],rows,'No peer volumes are hosted on this node.')}`)}</div>`;
  }
  function bindHostedRemoteVolumeActions(root=document) {
    $$('[data-hosted-volume-stop]',root).forEach(button=>button.onclick=()=>confirmAction('Stop peer volume access','Stop new filesystem operations for this hosted volume while preserving all stored data?',async()=>{await request(`/remote-volumes/hosted/${encodeURIComponent(button.dataset.hostedVolumeStop)}/stop`,{method:'POST',form:{}});await loadBackups();},false));
    $$('[data-hosted-volume-purge]',root).forEach(button=>button.onclick=()=>confirmAction('Remove hosted share','Remove the share metadata? The backing data directory will be preserved and can be reused if the same peer volume name is attached again.',async()=>{await request(`/remote-volumes/hosted/${encodeURIComponent(button.dataset.hostedVolumePurge)}`,{method:'DELETE'});await loadBackups();}));
    $$('[data-hosted-volume-delete]',root).forEach(button=>button.onclick=()=>confirmAction('Delete hosted volume data','Permanently delete this hosted peer volume and all of its stored files?',async()=>{await request(`/remote-volumes/hosted/${encodeURIComponent(button.dataset.hostedVolumeDelete)}?delete_data=true`,{method:'DELETE'});await loadBackups();}));
  }

  async function loadBackups() {
    if (!hasCap('backup-create')) {
      const [backups, peers, replicas, hostedVolumes] = await Promise.all([request('/backups'), request('/cluster/peers'), hasCap('replication-receiver')?request('/replications/replicas').catch(()=>[]):Promise.resolve([]), hasCap('remote-volume-receiver')?request('/remote-volumes/hosted').catch(()=>[]):Promise.resolve([])]);
      ...
      const replicasCard=hasCap('replication-receiver')?`<div class="mt-3">${card('Continuous replicas',`<div class="small text-secondary mb-3">Active replicas receive VM disk writes over authenticated WSS on this node's existing LiteVMM endpoint. Stopping replication on the source leaves the last synchronized disk here as a retained replica until it is explicitly purged.</div>${replicaInventoryTable(replicas)}`)}</div>`:'';
      $('#view').innerHTML=`<div class="mb-3 alert alert-info"><strong>Backup receiver profile.</strong> This node accepts archives and continuous disk replicas from authenticated paired LiteVMM hosts. It does not run virtual machines.</div>${card('Received backups',table(['VM','Archive','Size','Modified',''],rows,'No backups have been received yet.'),`<span class="small text-secondary">${(peers||[]).length} paired host${(peers||[]).length===1?'':'s'}</span>`)}${replicasCard}${hostedRemoteVolumeCard(hostedVolumes)}`;
      ...
      bindHostedRemoteVolumeActions();
      return;
    }
    const [backups, schedules, vms, peers, replicas, hostedVolumes] = await Promise.all([request('/backups'), request('/backups/schedules'), request('/vms'), request('/cluster/peers'), hasCap('replication-receiver')?request('/replications/replicas').catch(()=>[]):Promise.resolve([]), hasCap('remote-volume-receiver')?request('/remote-volumes/hosted').catch(()=>[]):Promise.resolve([])]);
