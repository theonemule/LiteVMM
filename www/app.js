(() => {
  ...
  async function loadNetworks(){
    ...
    $$('[data-overlay-validate]').forEach(b=>b.onclick=async()=>{const name=encodeURIComponent(b.dataset.overlayValidate);const [result,health]=await Promise.all([request(`/overlays/${name}`),request(`/overlays/${name}/health`)]);const checks=[['GOST process',result.tap_process],['TAP adapter',result.tap_type==='tap'],['TAP link up',result.tap_up],['Attached to bridge',result.tap_bridged],['Peer ARP/ICMP health probe',health.peer_reachable]];modal({eyebrow:'Layer-2 validation',title:result.name,body:`<div class="alert ${health.peer_reachable?'alert-success':'alert-danger'} small">${health.peer_reachable?'The authenticated WebSocket and Layer-2 path are passing traffic.':health.peer_reachable===null?'This overlay is active. Test ARP and ping between workloads, or stage both endpoints for a TAP address test.':'The peer TAP address did not answer. Stage both endpoints before testing.'}</div>${table(['Check','Result'],checks.map(([label,ok])=>`<tr><td>${esc(label)}</td><td>${ok?'<span class="text-success">Pass</span>':'<span class="text-danger">Fail</span>'}</td></tr>`),'')}<div class="small text-secondary mt-3">Staged validation sends ARP and ICMP through the TAP interfaces. Active overlays must be tested with workload addresses.</div>`});});
