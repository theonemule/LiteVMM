...
Both `/` and `/api/` are protected by the same HTTP Basic configuration. Debian installations use Nginx with PAM-backed local users. Alpine installations use lighttpd with an htpasswd file generated by `bootstrap-alpine.sh` from `VMAPI_HTTP_USER` and `VMAPI_HTTP_PASSWORD`. Visiting the console causes the browser's normal HTTP Basic login challenge. Once authenticated, the browser automatically reuses those same credentials for same-origin API calls. The JavaScript does **not** receive, store, or persist the user's password.
...
```bash
./bootstrap-debian.sh --admin YOUR_LOGIN --bridge br0
```

The bootstrap installs the minimal dependencies used by the project, including QEMU/KVM, OVMF, Docker, Nginx, fcgiwrap and the PAM module.

`--admin` adds a local PAM account to `vmapi-admin`. `--bridge` allows that existing Linux bridge through QEMU's bridge helper. Omit either option if you do not need it yet.

The default HTTP listener is **127.0.0.1:8080 only**. The Bootstrap console is at `/` and the JSON API is at `/api/`. Use an SSH tunnel while experimenting, or add TLS before exposing it to a network.

## Alpine installation

```bash
VMAPI_HTTP_USER=YOUR_LOGIN VMAPI_HTTP_PASSWORD='change-me' ./bootstrap-alpine.sh --admin YOUR_LOGIN --bridge br0
```

Run the bootstrap commands as root. The Alpine bootstrap enables the Alpine community repository if needed, installs QEMU/KVM, OVMF, Docker, GOST, lighttpd, fcgiwrap, and OpenRC service files, then exposes the console/API through lighttpd on `0.0.0.0:8080`.
...
After the upload, the script uses Alpine's `su root -c` form for the limited replacement operation: remove the configured destination, recreate it, extract the staged archive, run `apply-alpine-runtime.sh` from that validated destination, then restart `fcgiwrap-vmapi`, `websockify-vmapi`, `ttyd-vmapi`, `vmapi-console-gc`, and `lighttpd` in that order. The runtime update copies the source-controlled shell tools, CGI handlers (including `/peer-api/`), static web files, and OpenRC init scripts to the paths used by the Alpine bootstrap. It restarts `vmapi-overlay` only when it is already running. A failed required restart fails the sync. Docker and `vmapi-autostart` are intentionally untouched, so container and VM workloads are not disrupted.

The runtime update preserves host-specific configuration. It does not modify `/etc/vmapi/vmapi.conf`, `/etc/lighttpd/lighttpd.conf`, passwords, sudoers rules, users, groups, packages, or generated command wrappers. It atomically replaces only `/etc/lighttpd/conf.d/vmapi.conf` from the source-controlled VMAPI fragment, retaining the existing concrete noVNC web root when available and validating Lighttpd before activation.
...
Stop attached workloads before replacing a segment. Run `overlayctl reset` on each endpoint to stop and remove VMAPI's named TAP transports and generated hub routes, preserving paired hosts, bridges and workload configuration. Apply the updated runtime with `bash apply-alpine-runtime.sh`, then recreate the hub and spoke as above. Upgrade both peers together: peer API authentication has changed from request signatures to HTTP Basic.

### Minimal spoke without Lighttpd

On Alpine, `bash install-overlay-spoke.sh` installs only the pairing/overlay tools, GOST, networking dependencies and OpenRC supervisor. It installs no Lighttpd package or service. Perform the existing three-step `peerctl request`, `accept`, `complete` exchange at the CLI, passing each host name and endpoint. A headless spoke supplies its intended HTTP origin as pairing metadata; it serves no inbound API, so manage it locally. The full application can also act as a spoke while retaining its management UI/API; overlay operations on that node do not configure or restart Lighttpd.
