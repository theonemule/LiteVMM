...
RUN apk add --no-cache \
    ...
    lighttpd lighttpd-mod_auth apache2-utils fcgiwrap spawn-fcgi python3
...
COPY bin/peerctl bin/vmbackupctl bin/replicationctl bin/remote-volumectl bin/metricsctl bin/logctl bin/certctl /usr/local/bin/
COPY cgi/api.cgi cgi/peer-api.cgi cgi/remote-fs-ws.py /usr/lib/vmapi/cgi/
...
RUN chmod 0755 /usr/local/bin/peerctl /usr/local/bin/vmbackupctl /usr/local/bin/replicationctl /usr/local/bin/remote-volumectl /usr/local/bin/metricsctl /usr/local/bin/logctl /usr/local/bin/certctl \
    /usr/lib/vmapi/cgi/api.cgi /usr/lib/vmapi/cgi/peer-api.cgi /usr/lib/vmapi/cgi/remote-fs-ws.py /usr/local/bin/litevmm-backup-entrypoint \
 && mkdir -p /var/lib/vmapi/backups /var/lib/vmapi/replicas /var/lib/vmapi/replication-jobs /var/lib/vmapi/replication-receivers /var/lib/vmapi/remote-volumes /var/lib/vmapi/remote-volume-shares /var/lib/vmapi/peers /var/lib/vmapi/identity /run/vmapi /var/log/vmapi/backups \
