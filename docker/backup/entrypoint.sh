...
mkdir -p /var/lib/vmapi/{backups,backplane/peers,peers,identity,backup-jobs} /var/log/vmapi/backups /var/log/vmapi /run/vmapi /run/vmapi/conf.d /run/vmapi-backplane /etc/lighttpd
...
vmapi ALL=(root) NOPASSWD: /usr/local/bin/certctl status, /usr/local/bin/certctl issue *, /usr/local/bin/certctl renew, /usr/local/bin/certctl csr-generate *, /usr/local/bin/certctl csr-show, /usr/local/bin/certctl import-signed *, /usr/local/bin/certctl import-pair *, /usr/local/bin/certctl disable
SUDOERS
...
# Kernel NFS is deliberately loopback-only. A containerized backup-storage node therefore
# needs permission to mount the nfsd pseudo-filesystem; --privileged is the
# simplest supported deployment for this backup-server image.
...
if [[ $ready != true ]]; then
  ...
  echo 'LiteVMM storage backplane failed to start. The backup container requires access to the kernel NFS server (run it privileged).' >&2
  exit 1
fi
