...
grep -Fq 'LiteVMM peer volume over WSS' "$ROOT/www/app.js"
grep -Fq 'cgi.upgrade = "enable"' "$ROOT/lighttpd/vmapi.conf"
...
python3 -m py_compile "$ROOT/bin/litevmm-remote-fuse.py" "$ROOT/cgi/remote-fs-ws.py"
find "$ROOT" -type f \( -name '*.sh' -o -name 'vmctl' -o -name 'imagectl' -o -name 'netctl' -o -name 'overlayctl' -o -name 'dockerctl' -o -name 'dockerexecctl' -o -name 'docker-imagectl' -o -name 'docker-netctl' -o -name 'docker-volumectl' -o -name 'replicationctl' -o -name 'registryctl' -o -name 'metricsctl' -o -name 'certctl' -o -name 'consolectl' -o -name 'vmapi-console-gc' -o -name '*.cgi' \) -print0 |
...
bash "$ROOT/tests/remote-volumectl-test.sh"
bash "$ROOT/tests/remote-fs-wss-test.sh"
