...
exec "$@"
SH
cat > "$T/bin/backplanectl" <<'SH'
#!/usr/bin/env bash
set -Eeuo pipefail
[[ ${1:-} == path && ${3:-} == backups ]] || exit 2
mkdir -p "$FAKE_PEER_BACKUP_ROOT"
printf '%s\n' "$FAKE_PEER_BACKUP_ROOT"
SH
cp "$ROOT/bin/vmbackupctl" "$T/bin/vmbackupctl"
cp "$ROOT/bin/vmctl" "$T/bin/vmctl"
cp "$ROOT/bin/imagectl" "$T/bin/imagectl"
cp "$ROOT/bin/storagectl" "$T/bin/storagectl"
chmod +x "$T/bin/vmbackupctl" "$T/bin/vmctl" "$T/bin/imagectl" "$T/bin/storagectl" "$T/bin/backplanectl" "$T/bin/qemu-img" "$T/bin/install" "$T/bin/sudo"
...
export VMCTL="$T/bin/vmctl" IMAGECTL="$T/bin/imagectl" STORAGECTL="$T/bin/storagectl" VMAPI_BACKUP_ROOT="$T/backups" VMAPI_BACKUPCTL="$T/bin/vmbackupctl"
export VMAPI_BACKUP_JOB_ROOT="$T/backup-jobs" VMAPI_BACKUP_LOG_ROOT="$T/backup-logs"
export VMAPI_BACKPLANECTL="$T/bin/backplanectl" FAKE_PEER_BACKUP_ROOT="$T/peer-backups"

"$T/bin/vmctl" create demo --disk 1G --display none --network none >/dev/null
...
response=$(REQUEST_METHOD=GET PATH_INFO=/api/vms/demo/disks/1/download bash "$ROOT/cgi/api.cgi")
...
response=$(REQUEST_METHOD=GET PATH_INFO=/api/images/demo.iso IMAGECTL="$T/bin/imagectl" bash "$ROOT/cgi/api.cgi")
...
response=$(printf '%s' "$body" | REQUEST_METHOD=POST PATH_INFO=/api/storage CONTENT_TYPE=application/x-www-form-urlencoded \
  CONTENT_LENGTH=${#body} VMAPI_CONFIG="$T/vmapi.conf" STORAGECTL="$T/bin/storagectl" bash "$ROOT/cgi/api.cgi")
...
"$T/bin/vmbackupctl" create demo >/dev/null
...
"$T/bin/vmbackupctl" restore demo "$(basename "$archive")" >/dev/null
[[ -f "$T/vms/demo/vm.conf" ]]
[[ -f "$T/disks/demo/disk0.qcow2" && -f "$T/disks/demo/data.vmdk" ]]
...
for _ in $(seq 1 100); do
  job=$("$T/bin/vmbackupctl" job "$job_id")
...
response=$(printf '%s' "$body" | REQUEST_METHOD=POST PATH_INFO=/api/backups/start CONTENT_TYPE=application/x-www-form-urlencoded \
  CONTENT_LENGTH=${#body} BACKUPCTL="$T/bin/vmbackupctl" bash "$ROOT/cgi/api.cgi")
grep -q 'Status: 202 Accepted' <<< "$response"
cgi_job=$(sed -n 's/.*"id":"\([^"]*\)".*/\1/p' <<< "$response")
[[ -n $cgi_job ]]
for _ in $(seq 1 100); do
  response=$(REQUEST_METHOD=GET PATH_INFO="/api/backups/jobs/$cgi_job" BACKUPCTL="$T/bin/vmbackupctl" bash "$ROOT/cgi/api.cgi")
...
# A peer-targeted backup is written through the backplane and has no source-side copy.
peer_id=0123456789abcdef0123456789abcdef
peer_result=$("$T/bin/vmbackupctl" create demo --label peer --peer "$peer_id")
